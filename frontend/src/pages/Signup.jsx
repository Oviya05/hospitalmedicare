...
const Signup = () => {
  ...
  const handleFileInputChange = async (event) => {
    ...
    try {
      setLoading(true);
      const imageUrl = await uploadImageToCloudinary(file); // ✅ get the image URL

      console.log("Uploaded image URL:", imageUrl);

      setPreviewURL(imageUrl); // ✅ preview immediately
      setFormData({ ...formData, photo: imageUrl }); // ✅ save to form data
            ...
            <form onSubmit={submitHandler}>
              ...
              <div className="mb-5">
                <input
                  ...
                  className="w-full border px-4 py-3 rounded-md border-b border-solid border-[#0066ff61] focus:outline-none focus:border-b-primaryColor text-[16px] leading-7 text-headingColor placeholder:text-textColor"
                  required
                />
              </div>
              ...
              <div className="mt-7">
                ...
                >
                  {loading ? <HashLoader size={30} color="#ffffff" /> : "Signup"}
                </button>
              </div>
